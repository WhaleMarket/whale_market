...
function EditButton() {
    ...
    useEffect(() => {
        ...
        let editPossible = ProfileModificationState.profile.reduce(
            (count, value) => {
    ...
    const onSubmit = async () => {
        try {
            if (
                ProfileModificationState.profile[3].value !==
                InfoState.MyInformations[0].myImage
            ) {
                ...
                const profileEditData = {
                    user: {
                        username:
                            ProfileModificationState.profile[0].value !== ''
                                ? ProfileModificationState.profile[0].value
